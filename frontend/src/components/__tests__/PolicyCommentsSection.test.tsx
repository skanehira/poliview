...
describe('PolicyCommentsSection', () => {
  ...
  it('displays message when no comments exist', () => {
    ...
    expect(screen.getByText('まだコメントはありません。最初のコメントを投稿してみましょう！')).toBeInTheDocument()
  })
  ...
  it('renders comment form', () => {
    ...
    expect(screen.getByPlaceholderText('この政策に対するあなたの意見を投稿してください...')).toBeInTheDocument()
  ...
  it('submits comment with correct data', async () => {
    ...
    const textarea = screen.getByPlaceholderText('この政策に対するあなたの意見を投稿してください...')
    const submitButton = screen.getByRole('button', { name: 'コメントを投稿' })

    await user.type(textarea, 'This is my test comment')
    await user.click(submitButton)

    expect(mockOnAddComment).toHaveBeenCalledWith('1', 'This is my test comment', false)
  })
  ...
  it('submits anonymous comment', async () => {
    ...
    expect(mockOnAddComment).toHaveBeenCalledWith('1', 'Anonymous comment', true)
  })
